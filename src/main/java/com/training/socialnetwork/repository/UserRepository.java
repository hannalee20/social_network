...
public interface UserRepository extends JpaRepository<User, Integer> {
	...
	@Query(value = "" + "select * from user as u" + "where u.user_id <> :userId "
	...
	@Query(value = "" + "select count(p.post_id) as post, " + "count(c.comment_id) as comment, "
			+ "count(f1.user_id1 + f2.user_id2) as friend, " + "count(l.like_id) as likes " + "from user as u "
			+ "inner join post p on u.user_id = p.user_id " + "inner join comment c on u.user_id = c.user_id "
			+ "inner join friend f1 on u.user_id = f1.user_id1 " + "inner join friend f2 on u.user_id = f2.user_id2 "
